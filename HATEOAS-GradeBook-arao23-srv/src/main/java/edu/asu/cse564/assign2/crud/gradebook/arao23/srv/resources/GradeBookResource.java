...
import edu.asu.cse564.assign2.crud.gradebook.arao23.srv.model.GradeItem2;
        ...
        try {
            ...
            if (map.containsKey(id)) {
                ...
                if (!gradeAlreadyPresent) {
                    ...
                    //Addlinks to gradeitems
                    GradeItem2 gra2 = new GradeItem2();
                    gra2.setGradeItemName(gra.getGradeItemName());
                    gra2.setPercentageAllocation(gra.getPercentageAllocation());
                    gra2.setMarksObtained(gra.getMarksObtained());
                    gra2.setFeedback(gra.getFeedback());
                    gra2.setAppeal(gra.getAppeal());

                    //CREATE
                    gra2.addLink((URI.create(context.getAbsolutePath().toString())).toString(), "self");
                    //READ
                    gra2.addLink((URI.create(context.getAbsolutePath().toString())).toString() + "/"+gra2.getGradeItemName(), "READ");
                    //UPDATE
                    gra2.addLink((URI.create(context.getAbsolutePath().toString())).toString(), "Update");
                    //Delete
                    gra2.addLink((URI.create(context.getAbsolutePath().toString())).toString() + "/"+gra2.getGradeItemName(), "Delete");

                    response = Response.status(Response.Status.CREATED).location(locationURI).entity(gra2).build();
                }
                ...
                //adding for new gardeitm
                //List<GradeItem2> graItems2 = new ArrayList<>();
                //graItems2.add(gra);
                GradeItem2 gra2 = new GradeItem2();
                gra2.setGradeItemName(gra.getGradeItemName());
                gra2.setPercentageAllocation(gra.getPercentageAllocation());
            ...
            if (map.containsKey(id)) {
                ...
                for (GradeItem g : graItems) {
                    if (g.getGradeItemName().equals(gradeItemName)) {
                        //ADD object
                        GradeItem2 gra2 = new GradeItem2();
                        gra2.setGradeItemName(g.getGradeItemName());
                        gra2.setPercentageAllocation(g.getPercentageAllocation());
                        gra2.setMarksObtained(g.getMarksObtained());
                        gra2.setFeedback(g.getFeedback());
                        gra2.setAppeal(g.getAppeal());
                        
                        //READ
                        gra2.addLink((URI.create(context.getAbsolutePath().toString())).toString(), "self");
                        
                        response = Response.status(Response.Status.OK).entity(gra2).build();
                        gradeIemGot = true;
                        break;
                    }
                }
            ...
            if (map.containsKey(id)) {
                ...
                for (GradeItem g : graItems) {
                    if (g.getGradeItemName().equals(gradeItemName)) {
                        ...
                        GradeItem2 gra2 = new GradeItem2();
                       /* StringBuilder str=new StringBuilder("URI.create(context.getAbsolutePath().toString())).toString()");
                        int i=str.indexOf("/");
                        String ss= str.substring(i);
                        StringBuilder strfin= new StringBuilder(ss);
                        String finalstr=strfin.reverse().toString();
                        gra2.addLink(finalstr, "CREATE");*/
                       gra2.addLink((URI.create(context.getAbsolutePath().toString())).toString(), "self");
                        response = Response.status(Response.Status.NO_CONTENT).entity(gra2).build();
                        gradeIemGot = true;
                        break;
                    }
                }
